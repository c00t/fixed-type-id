...
mod kw {
    ...
    syn::custom_keyword!(rkyv_support);
}
...
impl<O: AttributeOptions> Parse for FilteredAttributes<O> {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        Ok(Self { options, other })
    }
}

impl<O: AttributeOptions> ToTokens for FilteredAttributes<O> {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        // Only output the non-revision attributes
...
impl Parse for ItemOption {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        if input.peek(kw::fixed_id_prefix) {
            return Ok(ItemOption::FixedIdPrefix(input.parse()?));
        }
        if input.peek(kw::default_version) {
            return Ok(ItemOption::DefaultVersion(input.parse()?));
        }
        if input.peek(kw::serde_support) {
            return Ok(ItemOption::SerdeSupport(input.parse()?));
        }
...
impl AttributeOptions for ItemOptions {
    ...
    fn finish(_path: Span, options: Vec<Self::Option>) -> syn::Result<Self> {
            ...
            match option {
                ItemOption::Revision(x) => {
                    if revision.is_some() {
                        return Err(Error::new(
                            x.key.span(),
                            "tried to set an option `revision` twice",
                        ));
                    }

                    revision = Some(x.value.base10_parse()?);
                }
                ItemOption::FixedIdPrefix(x) => {
                    if fixed_id_prefix.is_some() {
                        return Err(Error::new(
                            x.key.span(),
                            "tried to set an option `fixed_id_prefix` twice",
                        ));
                    }
                ...
                ItemOption::DefaultVersion(x) => {
                    if default_version.is_some() {
                        return Err(Error::new(
                            x.key.span(),
                            "tried to set an option `default_version` twice",
                        ));
                    }
                    default_version = Some(x.value.base10_parse()?);
                }
                ItemOption::SerdeSupport(x) => {
                    if serde_support.is_some() {
                        return Err(Error::new(
                            x.span(),
                            "tried to set an option `serde_support` twice",
                        ));
                    }
                    serde_support = Some(true);
                }
                ItemOption::RkyvSupport(x) => {
                    if rkyv_support.is_some() {
                        return Err(Error::new(
                            x.span(),
                            "tried to set an option `rkyv_support` twice",
                        ));
                    }
                    rkyv_support = Some(true);
                }
            }
        }
        ...
        Ok(Self {
            ...
            rkyv_support: rkyv_support.unwrap_or(false),
        })
    }
}
