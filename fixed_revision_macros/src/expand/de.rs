...
impl<'a, 'ast> Visit<'ast> for DeserializeVisitor<'a> {
    fn visit_enum(&mut self, i: &'ast Enum) -> syn::Result<()> {
        ...
        let mut enum_def = TokenStream::new();
        i.enum_.to_tokens(&mut enum_def);
        // convert to enum name relative to the current revision
        let enum_name = Ident::new(&format!("{}_{}", i.name, self.current), i.name.span());
        enum_name.to_tokens(&mut enum_def);
        i.generics.to_tokens(&mut enum_def);
        i.braces.surround(&mut enum_def, |tokens| {
            variants.to_tokens(tokens);
        });

        self.stream.append_all(enum_def);
        // self.stream.append_all(i.enum_.token);
    ...
    fn visit_struct(&mut self, i: &'ast Struct) -> syn::Result<()> {
        ...
        let (Fields::Named { ref fields, .. } | Fields::Unnamed { ref fields, .. }) = i.fields
        else {
            unreachable!();
        };
